<template>
  <header
    class="pt-[17px] px-[30px] pb-[23px] w-full flex flex-col sticky top-0 z-20 bg-white"
  >
    <div v-if="yScroll < 151" class="w-full justify-between items-center flex">
    ...
    <div class="flex">
      ...
      <span v-if="yScroll < 151" class="ml-auto">
        {{ appData.data.city.name }}
      </span>

      <div v-else class="flex gap-[30px] ml-auto">
        <a
          :href="`tel:${appData.data.phone}`"
          class="text-basic-black flex gap-[10px] items-center hover:underline hover:text-blue-800"
        >
          ...
          <span class="text-[16px] grow-0 shrink-0">
            {{ phone }}
          </span>
        </a>
        ...
        <a
          href="#form"
          class="py-[5px] px-[14px] rounded-[30px] bg-accent-orange text-white font-bold"
        >
          Вызвать мастера
        </a>
      </div>
